...
namespace WindowsFormsApplication
{
    public class AutoCompleteTextBox : TextBox
    {
        ...
        private void UpdateListBox()
        {
            ...
            {
                //MessageBox.Show("" + _values.Length);
                //String[] matches = Array.FindAll(_values, x => (x.StartsWith(word, StringComparison.OrdinalIgnoreCase) && !SelectedValues.Contains(x)));
                String[] matches = Array.FindAll(_values, x => (x.IndexOf(word, StringComparison.OrdinalIgnoreCase) >= 0) && !SelectedValues.Contains(x));
                if (matches.Length > 0)
                {
                    ShowListBox();
